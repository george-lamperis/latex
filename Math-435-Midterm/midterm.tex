...
In general, to calculate solutions:

You gotta do some stuff, thats for sure.
...
\begin{proof}
    ...
    \end{align*}
    Since 2 occurs on the right hand side, it must be the case that $p_1 = 2$.
    
    If $r \geq 2$, then $p_i$ is odd and $(p_i - 1)$ is even for $2 \leq i \leq
    r$.
    For the above equality to hold, it must be the case that $r=1$ and $p_1 =
    2$, i.e. $n=2^k$ with $k \geq 1.$
\end{proof}
