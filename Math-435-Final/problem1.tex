...
\subsection{1(g,h)}
\begin{lemma}
	Let $p$ be prime, $p > 3$. Then
\[
\left( \frac{3}{p} \right) =
\begin{cases}
	1  & \text{if } p \equiv 1, 5, 7 \pmod{24} \\
	-1 & \text{if } p \equiv 13, 17, 23 \pmod{24}
\end{cases}
\]
\end{lemma}
\begin{proof}
...
$p \equiv 9 \pmod{24}$. But $(3/p)$ is not defined, and so this case is
undefined as well.
...
For $p=2$ and $p=3$, this is undefined and so we are done.
